...
export function useSliceSelector(namespace, keys, deepCompare=false) {
  ...
  return useMapSelector(globalState => {
      ...
      let selectedStates = {};
      keys.forEach(key => selectedStates[key] = pageStates[key]);
...
export function mergeArrayAtIndex(arr, index, obj) {
  return [
    ...arr.slice(0, index),
    {...arr[index], ...obj},
    ...arr.slice(index + 1)
...
export function removeArrayAtIndex(arr, index, copy=true) {
  if(copy) arr = [...arr];
  arr.splice(index, 1);
  return arr;
}

export function insertArrayAtIndex(arr, index, obj, copy=true) {
  if(copy) arr = [...arr];
  if(index >= 0) {
